...
void backtracking(Lista<int> mosaicos, Nodo<int> *begin, int &casos);
...
int main(){
    ...
    backtracking(elementos,ptr,casos);
...
void backtracking(Lista<int> mosaicos, Nodo<int> *begin, int &casos)
{
    ...
    while(index <= 2)   // Dos opciones disponibles
    {
        if(validar(index,begin)){
            ...
            if((begin->getNext() == NULL && opcionValida == 1) || 
            (begin->getNext()->getNext() == NULL && opcionValida == 2)){

                casos = casos + 1;
            }else{
                backtracking(mosaicos,begin,casos);
            }
            begin = ant;
        }
        index++;
